...
INSERT INTO productos (categoria_id, codigo_barras, nombre, descripcion, precio_compra, precio_venta, stock_actual, stock_minimo, stock_maximo, unidad_medida, creado_por) SELECT 2, '7501234567996', 'Pluma de Gel Negra', 'Pluma de gel punta fina 0.5mm, tinta negra', 8.00, 15.00, 50, 10, 100, 'PIEZA', id FROM usuarios WHERE email = 'leonardo.lopez@papeleriasmart.com';
...
SELECT * FROM ventas WHERE fecha_venta BETWEEN '2025-01-01' AND '2025-12-31';

SELECT v.id, v.cliente_nombre, v.fecha_venta, v.total, COUNT(dv.id) AS productos_vendidos 
FROM ventas v INNER JOIN detalles_venta dv ON v.id = dv.venta_id WHERE v.fecha_venta BETWEEN '2026-01-01' AND '2026-12-31' GROUP BY v.id;

SELECT DATE_FORMAT(fecha_venta, '%Y-%m') AS mes, COUNT(*) AS num_ventas, SUM(total) AS ingresos_totales 
FROM ventas WHERE fecha_venta BETWEEN '2026-01-01' AND '20256-12-31' GROUP BY mes ORDER BY mes;

-- Cambia las fechas por las que necesites
SELECT * FROM ventas WHERE fecha_venta BETWEEN '2025-07-01' AND '2025-07-31';
